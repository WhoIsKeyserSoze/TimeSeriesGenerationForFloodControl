...
model_path = r".\BILSTM_module\BILSTM_storage"
model = 'notloaded'
isLoaded = False
...
# If the measure_list does not make at least 30 measures, the LSTM network won't be able to make predictions
def PredictFromList(measure_list, pred_len) :

    #checking measure_list size
    if(len(measure_list) < 30) :
        Warning.warn("TimeSeriesGenerator.LSTM.PredictFromList : Not enought data. Make sure to give at least 30 measures")
        return []
